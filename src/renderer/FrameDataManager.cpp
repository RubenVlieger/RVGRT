...
CameraData FrameDataManager::PrepareCameraData(const Character& character) {
    ...
    // Position
    camData.position = simd_make_float3(
        static_cast<float>(character.position.x),
        static_cast<float>(character.position.y),
        static_cast<float>(character.position.z)
    );
    
    // Forward direction
    camData.forward = simd_make_float3(
        static_cast<float>(character.direction.x),
    ...
    float aspect = static_cast<float>(State::dispWIDTH) / static_cast<float>(State::dispHEIGHT);
    ...
    memcpy(&camData.prevUnjitteredViewProjection,
           &character.lastRenderedViewProjectionMatrix,
           sizeof(simd_float4x4));
    
    return camData;
}

FrameData FrameDataManager::PrepareFrameData(unsigned int frameCount, simd_int3 worldOrigin) {
    FrameData frameData;
    ...
    if (_firstFrame) {
        ...
        // Use a minimum delta time to avoid issues with pauses/debugging
        _deltaTime = fmaxf(1.0f / 60.0f, 0.001f);
    }
...
void FrameDataManager::AppendCharacterData(CharacterGPUData& data, const Character& character, int& activeCount) {
    ...
    memcpy(&data.invBoundingBoxes[idx],
           &character.boundingBox.inverseModelMatrix,
           sizeof(simd_float4x4));
    
    // Body parts (6 per character: head, trunk, left arm, right arm, left leg, right leg)
    memcpy(&data.invBodyParts[idx * 6 + 0],
           &character.head.inverseModelMatrix,
           sizeof(simd_float4x4));
    memcpy(&data.invBodyParts[idx * 6 + 1],
           &character.trunk.inverseModelMatrix,
           sizeof(simd_float4x4));
    memcpy(&data.invBodyParts[idx * 6 + 2],
           &character.leftArm.inverseModelMatrix,
           sizeof(simd_float4x4));
    memcpy(&data.invBodyParts[idx * 6 + 3],
           &character.rightArm.inverseModelMatrix,
           sizeof(simd_float4x4));
    memcpy(&data.invBodyParts[idx * 6 + 4],
           &character.leftLeg.inverseModelMatrix,
           sizeof(simd_float4x4));
    memcpy(&data.invBodyParts[idx * 6 + 5],
           &character.rightLeg.inverseModelMatrix,
           sizeof(simd_float4x4));
    
    activeCount++;
}

CharacterGPUData FrameDataManager::PrepareCharacterData(const Character& player, 
