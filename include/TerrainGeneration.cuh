...
__device__ __forceinline__ float simplex2D(float px, float py) {
...
__device__ __forceinline__ float simplex3D(float px, float py, float pz) 
{
...
__device__ __forceinline__ float fbm3D(float x, float y, float z, int octaves, float frequency, float lacunarity, float persistence) {
    float total = 0.0f;
...
__device__ __forceinline__ float fbm2D(float x, float z, int octaves, float frequency, float lacunarity, float persistence) {
    float total = 0.0f;
    float amplitude = 1.0f;
    for (int i = 0; i < octaves; i++) {
...
__device__ __forceinline__ float Evaluate( float x, float y, float z) {
    ...
    // ** Biome Generation **
    const float BIOME_FREQUENCY = 0.005f;          // Controls the size of biomes. Smaller value = larger biomes.
