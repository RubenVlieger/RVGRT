...
inline uint ReadIndirection(SVO_TEXTURE_TYPE indirection, uint3 pos, int indW,
                            int indH, int indD) {
  (void)indW;
  (void)indH;
  (void)indD; // Unused in Metal path
  return indirection.read(pos).r;
}

inline bool GetStepPos(thread int3 &ipos, float3 dir,
                       SVO_TEXTURE_TYPE indirection, SECTOR_BUFFER_TYPE sectors,
                       OCCUPANCY_TYPE occupancy, DATA_TYPE data,
                       MASK_TYPE sectorMasks, int3 worldOrigin,
                       thread uint8_t &outMatID, int indW, int indH, int indD,
                       bool lodOnly = false) {
...
inline hitInfo trace(float3 rayPos, float3 rayDir, SVO_TEXTURE_TYPE indirection,
                     SECTOR_BUFFER_TYPE sectors, OCCUPANCY_TYPE occupancy,
                     DATA_TYPE data, MASK_TYPE sectorMasks, int3 worldOrigin,
                     int indW, int indH, int indD,
                     CHAR_DATA_TYPE charData = nullptr, int maxIters = 512,
                     bool lodOnly = false) {
  hitInfo hit;
  hit.hit = false;
  ...
  if (charData != nullptr) {
    ...
    for (int i = 0; i < charData->numCharacters; ++i) {
      ...
      safeLocalBDir.x = (abs(safeLocalBDir.x) < 1e-8f)
                            ? copysign(1e-8f, safeLocalBDir.x)
                            : safeLocalBDir.x;
      safeLocalBDir.y = (abs(safeLocalBDir.y) < 1e-8f)
                            ? copysign(1e-8f, safeLocalBDir.y)
                            : safeLocalBDir.y;
      safeLocalBDir.z = (abs(safeLocalBDir.z) < 1e-8f)
                            ? copysign(1e-8f, safeLocalBDir.z)
                            : safeLocalBDir.z;
      ...
      for (int p = 0; p < 6; ++p) {
        ...
        safeLocalDir.x = (abs(safeLocalDir.x) < 1e-8f)
                             ? copysign(1e-8f, safeLocalDir.x)
                             : safeLocalDir.x;
        safeLocalDir.y = (abs(safeLocalDir.y) < 1e-8f)
                             ? copysign(1e-8f, safeLocalDir.y)
                             : safeLocalDir.y;
        safeLocalDir.z = (abs(safeLocalDir.z) < 1e-8f)
                             ? copysign(1e-8f, safeLocalDir.z)
                             : safeLocalDir.z;
        ...
        if (tNear <= tFar && tFar > 0.0f && tNear < closestCharT) {
          ...
          closestCharNormal =
              normalize((transpose(invPart) * float4(localNormal, 0.0f)).xyz);
        }
      }
    }
#endif
  ...
  for (int i = 0; i < maxIters; ++i) {
    ...
    bool stepped =
        GetStepPos(voxelPos, safeDir, indirection, sectors, occupancy, data,
                   sectorMasks, worldOrigin, matID, indW, indH, indD, lodOnly);
...
inline bool traceShadow(float3 rayPos, float3 rayDir, float maxDist,
  ...
  for (int i = 0; i < maxIters; ++i) {
    ...
    bool stepped =
        GetStepPos(voxelPos, safeDir, indirection, sectors, occupancy, data,
                   sectorMasks, worldOrigin, matID, indW, indH, indD);
...
// Helper functions for float3 min/max (fminf/fmaxf don't work with float3 in
// CUDA)
...
__device__ inline int popcnt64(ulong mask) { return __popcll(mask); }
...
GetStepPos(int3 &ipos, float3 dir, SVO_TEXTURE_TYPE indirection,
           SECTOR_BUFFER_TYPE sectors, OCCUPANCY_TYPE occupancy, DATA_TYPE data,
           MASK_TYPE sectorMasks, int3 worldOrigin, uint8_t &outMatID, int indW,
           int indH, int indD, bool lodOnly = false) {
  if (ipos.y < 0 || ipos.y >= int(SIZEY)) {
    AlignToCellBoundaries(ipos, dir, 32);
    return true;
  }
  ...
  uint3 localPos =
      make_uint3((uint)ipos.x & 31u, (uint)ipos.y & 31u, (uint)ipos.z & 31u);
  uint3 brickRel = (localPos >> 3) & 3;
  uint brickLinearIdx = GetLinearIndex4(brickRel);
...
__device__ inline hitInfo
trace(float3 rayPos, float3 rayDir, SVO_TEXTURE_TYPE indirection,
      SECTOR_BUFFER_TYPE sectors, OCCUPANCY_TYPE occupancy, DATA_TYPE data,
      MASK_TYPE sectorMasks, int3 worldOrigin, int indW, int indH, int indD,
      CHAR_DATA_TYPE charData = nullptr, int maxIters = 512,
      bool lodOnly = false) {
  hitInfo hit;
  hit.hit = false;
  ...
  float3 safeDir = rayDir;
  safeDir.x =
      (fabsf(safeDir.x) < 1e-8f) ? copysignf(1e-8f, safeDir.x) : safeDir.x;
  safeDir.y =
      (fabsf(safeDir.y) < 1e-8f) ? copysignf(1e-8f, safeDir.y) : safeDir.y;
  safeDir.z =
      (fabsf(safeDir.z) < 1e-8f) ? copysignf(1e-8f, safeDir.z) : safeDir.z;
  float3 invDir =
      make_float3(1.0f / safeDir.x, 1.0f / safeDir.y, 1.0f / safeDir.z);

  float3 worldMin = make_float3(worldOrigin.x * 32.0f, worldOrigin.y * 32.0f,
                                worldOrigin.z * 32.0f);
  ...
  float3 startPos = ClipRayToAABB(rayPos, safeDir, invDir, worldMin, worldMax);
  int3 voxelPos = make_int3((int)floorf(startPos.x), (int)floorf(startPos.y),
  ...
  if (charData != nullptr) {
    ...
    for (int i = 0; i < charData->numCharacters; ++i) {
      ...
      float3 safeLocalBDir = localBDir;
      safeLocalBDir.x = (fabsf(safeLocalBDir.x) < 1e-8f)
                            ? copysignf(1e-8f, safeLocalBDir.x)
                            : safeLocalBDir.x;
      safeLocalBDir.y = (fabsf(safeLocalBDir.y) < 1e-8f)
                            ? copysignf(1e-8f, safeLocalBDir.y)
                            : safeLocalBDir.y;
      safeLocalBDir.z = (fabsf(safeLocalBDir.z) < 1e-8f)
                            ? copysignf(1e-8f, safeLocalBDir.z)
                            : safeLocalBDir.z;
      float3 invLocalBDir =
          make_float3(1.0f / safeLocalBDir.x, 1.0f / safeLocalBDir.y,
                      1.0f / safeLocalBDir.z);
      ...
      for (int p = 0; p < 6; ++p) {
        ...
        float3 safeLocalDir = localDir;
        safeLocalDir.x = (fabsf(safeLocalDir.x) < 1e-8f)
                             ? copysignf(1e-8f, safeLocalDir.x)
                             : safeLocalDir.x;
        safeLocalDir.y = (fabsf(safeLocalDir.y) < 1e-8f)
                             ? copysignf(1e-8f, safeLocalDir.y)
                             : safeLocalDir.y;
        safeLocalDir.z = (fabsf(safeLocalDir.z) < 1e-8f)
                             ? copysignf(1e-8f, safeLocalDir.z)
                             : safeLocalDir.z;
        float3 invLocalDir =
            make_float3(1.0f / safeLocalDir.x, 1.0f / safeLocalDir.y,
                        1.0f / safeLocalDir.z);
        ...
        if (tNear <= tFar && tFar > 0.0f && tNear < closestCharT) {
          ...
          float3 hitPosLocal =
              make_float3(localPos.x + localDir.x * closestCharT,
                          localPos.y + localDir.y * closestCharT,
                          localPos.z + localDir.z * closestCharT);
          float3 absHit = make_float3(
              fabsf(hitPosLocal.x), fabsf(hitPosLocal.y), fabsf(hitPosLocal.z));
          float3 localNormal = make_float3(0.0f, 0.0f, 0.0f);
          if (absHit.x > absHit.y && absHit.x > absHit.z)
            localNormal =
                make_float3(copysignf(1.0f, hitPosLocal.x), 0.0f, 0.0f);
          else if (absHit.y > absHit.z)
            localNormal =
                make_float3(0.0f, copysignf(1.0f, hitPosLocal.y), 0.0f);
          else
            localNormal =
                make_float3(0.0f, 0.0f, copysignf(1.0f, hitPosLocal.z));

          mat4 transposed = mat4_transpose(invPart);
          closestCharNormal =
              normalize(mat4_mul_float3(transposed, localNormal, false));
        }
      }
    }
#endif
  ...
  for (int i = 0; i < maxIters; ++i) {
    ...
    if (!stepped) {
      ...
      float3 cellMin =
          make_float3((float)voxelPos.x, (float)voxelPos.y, (float)voxelPos.z);
      ...
      if (closestCharT < tEntry) {
        ...
        hit.normal = make_half3(closestCharNormal.x, closestCharNormal.y,
                                closestCharNormal.z);
        hit.matID = closestCharMatID;
      ...
      float3 center =
          make_float3(cellMin.x + 0.5f, cellMin.y + 0.5f, cellMin.z + 0.5f);
      float3 d = make_float3(hit.pos.x - center.x, hit.pos.y - center.y,
                             hit.pos.z - center.z);
      ...
      float3 fpos =
          make_float3(floorf(hit.pos.x), floorf(hit.pos.y), floorf(hit.pos.z));
      float3 localPos = make_float3(hit.pos.x - fpos.x, hit.pos.y - fpos.y,
                                    hit.pos.z - fpos.z);
...
inline uint ReadIndirection(const uint32_t *indirection, uint3 pos, int indW,
                            int indH, int indD) {
  (void)indH;
  (void)indD; // Unused but kept for API consistency
  return indirection[pos.x + pos.y * indW + pos.z * indW * indH];
}

inline bool GetStepPos(int3 &ipos, float3 dir, const uint32_t *indirection,
                       const SectorInfo *sectors, const uint64_t *occupancy,
                       const uint8_t *data, const uint64_t *sectorMasks,
                       int3 worldOrigin, uint8_t &outMatID, int indW, int indH,
                       int indD, bool lodOnly = false) {
...
inline hitInfo trace(float3 rayPos, float3 rayDir, const uint32_t *indirection,
                     const SectorInfo *sectors, const uint64_t *occupancy,
                     const uint8_t *data, const uint64_t *sectorMasks,
  ...
  float3 safeDir = rayDir;
  safeDir.x = (std::abs(safeDir.x) < 1e-8f) ? std::copysign(1e-8f, safeDir.x)
                                            : safeDir.x;
  safeDir.y = (std::abs(safeDir.y) < 1e-8f) ? std::copysign(1e-8f, safeDir.y)
                                            : safeDir.y;
  safeDir.z = (std::abs(safeDir.z) < 1e-8f) ? std::copysign(1e-8f, safeDir.z)
                                            : safeDir.z;
  float3 invDir =
      make_float3(1.0f / safeDir.x, 1.0f / safeDir.y, 1.0f / safeDir.z);
  ...
  float3 startPos = ClipRayToAABB(rayPos, safeDir, invDir, worldMin, worldMax);
  int3 voxelPos =
      make_int3((int)std::floor(startPos.x), (int)std::floor(startPos.y),
                (int)std::floor(startPos.z));
  ...
  if (charData != nullptr) {
    ...
    for (int i = 0; i < charData->numCharacters; ++i) {
      ...
      float3 safeLocalBDir = localBDir;
      safeLocalBDir.x = (std::abs(safeLocalBDir.x) < 1e-8f)
                            ? std::copysign(1e-8f, safeLocalBDir.x)
                            : safeLocalBDir.x;
      safeLocalBDir.y = (std::abs(safeLocalBDir.y) < 1e-8f)
                            ? std::copysign(1e-8f, safeLocalBDir.y)
                            : safeLocalBDir.y;
      safeLocalBDir.z = (std::abs(safeLocalBDir.z) < 1e-8f)
                            ? std::copysign(1e-8f, safeLocalBDir.z)
                            : safeLocalBDir.z;
      float3 invLocalBDir =
          make_float3(1.0f / safeLocalBDir.x, 1.0f / safeLocalBDir.y,
                      1.0f / safeLocalBDir.z);
      ...
      for (int p = 0; p < 6; ++p) {
        ...
        float3 safeLocalDir = localDir;
        safeLocalDir.x = (std::abs(safeLocalDir.x) < 1e-8f)
                             ? std::copysign(1e-8f, safeLocalDir.x)
                             : safeLocalDir.x;
        safeLocalDir.y = (std::abs(safeLocalDir.y) < 1e-8f)
                             ? std::copysign(1e-8f, safeLocalDir.y)
                             : safeLocalDir.y;
        safeLocalDir.z = (std::abs(safeLocalDir.z) < 1e-8f)
                             ? std::copysign(1e-8f, safeLocalDir.z)
                             : safeLocalDir.z;
        float3 invLocalDir =
            make_float3(1.0f / safeLocalDir.x, 1.0f / safeLocalDir.y,
                        1.0f / safeLocalDir.z);
        ...
        if (tNear <= tFar && tFar > 0.0f && tNear < closestCharT) {
          ...
          float3 hitPosLocal =
              make_float3(localPos.x + localDir.x * closestCharT,
                          ...
                          localPos.z + localDir.z * closestCharT);
          float3 absHit =
              make_float3(std::abs(hitPosLocal.x), std::abs(hitPosLocal.y),
                          std::abs(hitPosLocal.z));
          float3 localNormal = make_float3(0.0f, 0.0f, 0.0f);
          if (absHit.x > absHit.y && absHit.x > absHit.z)
            localNormal =
                make_float3(std::copysign(1.0f, hitPosLocal.x), 0.0f, 0.0f);
          else if (absHit.y > absHit.z)
            localNormal =
                make_float3(0.0f, std::copysign(1.0f, hitPosLocal.y), 0.0f);
          else
            localNormal =
                make_float3(0.0f, 0.0f, std::copysign(1.0f, hitPosLocal.z));
  ...
  for (int i = 0; i < maxIters; ++i) {
    ...
    if (!stepped) {
      ...
      float3 fpos = make_float3(std::floor(hit.pos.x), std::floor(hit.pos.y),
                                std::floor(hit.pos.z));
      float3 localPos = make_float3(hit.pos.x - fpos.x, hit.pos.y - fpos.y,
                                    hit.pos.z - fpos.z);
    ...
    nextVoxelPos.x = (safeDir.x >= 0)
                         ? std::max(nextVoxelPos.x, originalVoxelPos.x)
                         : std::min(nextVoxelPos.x, originalVoxelPos.x);
    nextVoxelPos.y = (safeDir.y >= 0)
                         ? std::max(nextVoxelPos.y, originalVoxelPos.y)
                         : std::min(nextVoxelPos.y, originalVoxelPos.y);
    nextVoxelPos.z = (safeDir.z >= 0)
                         ? std::max(nextVoxelPos.z, originalVoxelPos.z)
                         : std::min(nextVoxelPos.z, originalVoxelPos.z);
